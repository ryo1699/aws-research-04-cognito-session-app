...
この実装では 60 分で自動ログアウトする課題に合わせて、refresh token の有効期限も 60 分にし、さらに Next.js 側では refresh 処理を実装していません。つまり、token を自動更新して session を延長しない設計です。
...
ただし、そのまま JSON を入れるのではなく、AES-256-GCM で暗号化して保存します。
...
`createAppSession` は session の期限を次の短い方にします。

```text
min(Cognito accessToken の期限, SESSION_MAX_AGE_SECONDS)
```
...
### 3. Next.js が API Gateway に accessToken を送る

Cookie の復号に成功した場合、Next.js は session 内の `accessToken` を API Gateway に送ります。
...
### 4. API Gateway JWT authorizer が token を検証する
...
60 分ログアウトは 2 つの層で実現しています。

### Cognito token の期限

Terraform の User Pool Client で token 期限を 60 分にしています。
...
`token_validity_units` は minutes です。

### Cookie session の期限

Next.js 側では `.env.local` の `SESSION_MAX_AGE_SECONDS=3600` を使います。Cookie の `Max-Age` も 3600 秒です。
...
### refresh token を使わない理由

refresh token を使うと、access token が切れても新しい token を取得してログイン状態を延長できます。しかし課題は「60 分で自動ログアウト」なので、この実装では refresh 処理を入れていません。
...
`accessToken` は 60 分で失効します。盗まれても、期限が切れれば API Gateway が `401` を返します。

資料の例でも、期限切れ token を curl で送ると `401 Unauthorized` になります。
...
| Token | 使い道 | この実装での扱い |
| --- | --- | --- |
| `accessToken` | API Gateway に送って API 認可に使う。 | Cookie session 内に暗号化保存し、API Gateway への `Authorization` header に付ける。 |
| `idToken` | ユーザー情報の確認に使う。 | Cookie session 内に保存可能。API 認可には使わない。 |
| `refreshToken` | token 更新に使う。 | Cognito 側の期限は 60 分。Next.js 側では refresh 処理を実装しない。 |

60 分ログアウトを成立させるには、access token だけでなく refresh token と Cookie session の期限も考える必要があります。
...
refresh token を使わないため、ユーザー体験としては 60 分後に再ログインが必要です。これは課題の「60 分で自動ログアウト」を優先した設計です。
...
API Gateway は Cognito JWT authorizer で token を検証し、正しい token だけを Lambda に通します。Lambda は DynamoDB の dummy data を返します。token と Cookie の期限を 60 分にそろえ、refresh 処理を入れないことで、60 分で自動ログアウトする挙動を作っています。
